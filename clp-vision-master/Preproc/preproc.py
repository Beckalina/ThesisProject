...
class TaskFunctions(object):
    ...
    def exec_task(self, task):
        ...
        if fn is None:
            print('%s is an unknown task' % task)
            sys.exit(1)
        else:
            fn()
    ...
    def tsk_saiapr(self):
        ...
        if args.notag:
            refdf = refdf[['i_corpus', 'image_id', 'region_id', 'r_corpus', 'rex_id', 'refexp']]
        else:
            refdf['tagged'] = refdf['refexp'].apply(postag)
            refdf = refdf[['i_corpus', 'image_id', 'region_id', 'r_corpus', 'rex_id', 'refexp', 'tagged']]

        TaskFunctions._dumpDF(refdf, args.out_dir + '/saiapr_refdf.json', args)

        # load and write out the splits on SAIAPR as used by Berkeley group (50/50)
        b_splits_train_p = config.get('SAIAPR', 'berkeley_splits_path') + '/referit_trainval_imlist.txt'
        b_splits_test_p = config.get('SAIAPR', 'berkeley_splits_path') + '/referit_test_imlist.txt'
        ...
        saiapr_berkeley_splits = {
            'test': saiapr_test_files.tolist(),
            'train': saiapr_train_files.tolist()
        }
        ...
        saiapr_train_90 = saiapr_train_files.tolist() + saiapr_test_files.tolist()[:8000]
        saiapr_test_90 = saiapr_test_files.tolist()[8000:]
        saiapr_90_10_splits = {
            'test': saiapr_test_90,
    ...
    def _process_refcoco(inpath, outbase, targs):
        ...
        for n, this_row in refcocodf.iterrows():
            ...
            for this_sentence, this_rexid in zip(this_row['sentences'], this_row['sent_ids']):
                this_sentence_sent = this_sentence['sent']
                refdf_list.append((this_file, this_region, this_sentence_sent, this_rexid))
    ...
    def tsk_refcoco(self):
        ...
        TaskFunctions._process_refcoco(refcoco_path, 'refcoco_refdf', targs)
    ...
    def tsk_refcocoplus(self):
        ...
        TaskFunctions._process_refcoco(refcocoplus_path, 'refcocoplus_refdf', targs)
    ...
    def tsk_mscocobb(self):
        # 2019-03-13: I have removed the restriction to extracting only the files that are in refcoco and grex,
        # to get all objects. I'm leaving the code commented in here for now, as I don't know right now whether
        # this might change anything for old downstream functions.
        #
        # 2019-03-13: TODO. This should actually run over validation as well. See guesswhat.
